...
async function applyDocuments(db,documents){
  ...
  await db.doc("migrations/no-space-v1").set({status:"complete",sourceSpace:NO_SPACE_PRODUCTION_SOURCE,completedAt:new Date().toISOString(),documentCount:documents.length},{merge:false});
}
...
try{
  ...
  console.log(JSON.stringify({mode:options.apply?"APPLY":"DRY RUN",project:options.project,sourceSpace:options.source_space,counts:result.counts,warnings:result.warnings,blockers:result.blockers},null,2));
